...
class ImageToolApp(tk.Tk):
    def __init__(self):
        ...
        self.dir_path.grid(row=0, column=0, columnspan=3,
                           sticky="ew", padx=5, pady=5)
        self.dir_path.insert(0, os.getcwd())
        # tk.Buttons

        tk.Button(self.file_frm, bd=8, bg="cyan", text="Use Path",
                  command=self.use_path).grid(row=2, column=0)
        tk.Button(self.file_frm, bd=8, bg="light green", text="Browse",
                  command=self.browse_path).grid(row=2, column=1, padx=5, pady=5)
        tk.Button(self.file_frm, bd=8, bg="light blue", text="Save Bookmark",
                  command=self.save_bookmarks).grid(row=2, column=2, padx=5, pady=5)
        tk.Button(self.file_frm, bd=8, bg="light yellow", text="Delete Bookmark",
                  command=self.delete_bookmark).grid(row=2, column=3, padx=5, pady=5)

        # Bookmarks Combobox
        self.bookmark_var = StringVar()
        self.bookmark_combo = ttk.Combobox(
            self.file_frm, textvariable=self.bookmark_var, values=list(self.bookmarks.keys()))
        ...
        tk.Button(self.file_frm, bd=10, bg="orange", text="Open Selected Path",
                  command=self.open_selected_path).grid(row=3, column=2, padx=5, pady=5)
        tk.Button(self.file_frm, bd=8, bg="light pink", text="Add Bookmark",
                  command=self.add_bookmark).grid(row=3, column=3, padx=5, pady=5)
        ...
        self.progress_label = tk.Label(
            self.top, text="if loading please wait...")
        self.progress_label.grid(row=2, column=2)

        self.canvas = tk.Canvas(self.top, bg="white", height=200,
                                width=600, xscrollincrement=20, yscrollincrement=20)
        self.canvas.grid(row=4, column=2, columnspan=8,
        ...
        self.framebottom.grid(row=8, column=1, columnspan=2,
                              sticky="ew", padx=10, pady=10)
        self.btn_browse = tk.Button(
            self.framebottom, bd=8, bg="azure", text="Get Images", command=self.get_images)
        ...
        self.check_recursive = tk.Checkbutton(
            self.framebottom, text="Recursive View", variable=self.var_recursive, command=self.update_mode_label)
        self.check_recursive.grid(row=0, column=1, padx=5)
        self.btn_view_full = tk.Button(
            self.framebottom, bd=8, bg="medium orchid", text="View Full Image", command=self.view_full_image)
        self.btn_view_full.grid(row=0, column=2, padx=5)

        self.btn_unselect = tk.Button(
            self.framebottom,  bd=8, bg="SeaGreen2", text="Unselect", command=self.unselect_image)
        self.btn_unselect.grid(row=0, column=3, padx=5)

        self.btn_merge = tk.Button(
            self.framebottom, bd=8, bg="bisque", text="Merge Images", command=self.merge_images)
        self.btn_merge.grid(row=0, column=4, padx=5)
        self.save_merge = tk.Button(self.framebottom, bd=8, bg="bisque",
                                    text="Savr Merged Image", command=self.save_merged_image)
        self.save_merge.grid(row=0, column=7, padx=5)

        self.btn_sprite = tk.Button(self.framebottom, bd=8, bg="light steel blue",
                                    text="Build Sprite Sheet", command=self.build_sprite_sheet)
        self.btn_sprite.grid(row=0, column=5, padx=5)
        self.btn_convert = tk.Button(
            self.framebottom, bd=6, bg="burlywood", text="Convert to PNG", command=self.convert_to_png)
        self.btn_convert.grid(row=0, column=6, padx=5)
        self.frame_right = tk.Frame(self)
        self.frame_right.grid(row=5, column=3, sticky="nsew", padx=10, pady=10)

        self.lbl_info = tk.Label(
            self.frame_right, text="File Info", anchor="w")
        self.lbl_info.grid(row=0, column=5, sticky="w", pady=5)

        self.txt_info = tk.Text(self.frame_right, bd=7,
                                height=8, width=40, wrap="word")
        self.txt_info.grid(row=1, column=6, sticky="nsew")

        self.lbl_selected = tk.Label(
            self.frame_right, bd=10, text="Selected Images")
        self.lbl_selected.grid(row=2, column=0, sticky="w", pady=5)

        self.listbox_selected = tk.Listbox(
            self.frame_right, bd=9, height=15, width=40)
    ...
    def add_bookmark(self):
        ...
        if not os.path.exists(path):
            messagebox.showerror(
                "Save Bookmark", "The selected path does not exist!")
            return
        ...
        if not name:
            messagebox.showwarning(
                "Save Bookmark", "Bookmark name cannot be empty!")
            return

        if name in self.bookmarks:
            overwrite = messagebox.askyesno(
                "Overwrite Bookmark", f"A bookmark with the name '{name}' already exists. Overwrite?")
            if not overwrite:
                return
        ...
        messagebox.showinfo(
            "Bookmark Saved", f"Bookmark '{name}' saved successfully.")
    ...
    def delete_bookmark(self):
        ...
        if selected in self.bookmarks:
            ...
            self.update_bookmarks()
            messagebox.showinfo("Bookmark Deleted",
                                f"Bookmark '{selected}' deleted successfully.")
    ...
    def update_bookmarks(self):
        self.bookmark_combo['values'] = list(
            self.bookmarks.keys())  # Update the dropdown
    ...
    def load_thumbnail(self):
        ...
        if self.thumbnail_index < len(self.image_paths):
            ...
            try:
                ...
                frame.grid(row=self.thumbnail_index // self.thumbnails_per_row,
                ...
                # Add the file name
                lbl_info = tk.Label(frame, text=os.path.basename(
                    img_path), wraplength=120, anchor="center")
...
# lbl_img.bind("<Double-1>", lambda e, path=img_path: self.add_to_selection(path))
    ...
    def unselect_image(self):
        ...
        if not selected_idx:
            messagebox.showwarning(
                "No Selection", "Select an image to unselect.")
            return

        selected_file = self.listbox_selected.get(selected_idx)
        self.listbox_selected.delete(selected_idx)
        self.selected_images = [
            img for img in self.selected_images if os.path.basename(img) != selected_file]
    ...
    def build_sprite_sheet(self):
        ...
        try:
            ...
            sprite_sheet = Image.new(
                'RGBA', (img_width * columns, img_height * rows))
            ...
            messagebox.showinfo("Sprite Sheet Complete",
                                "Sprite sheet built successfully.")
        except Exception as e:
    ...
    def convert_to_png(self):
        ...
        if success_count > 0:
            messagebox.showinfo(
                "Conversion Complete", f"Successfully converted {success_count} files to PNG.")
        if error_files:
            messagebox.showerror(
                "Conversion Errors", f"Failed to convert the following files:\n{error_files}")
