...
class ImageViewer:
    ...
    def navigate(self, direction):
        ...
        # Update the current index and display the image
    ...
    def add_navigation_buttons(self):
        ...
        if self.image_paths:  # Only add buttons if there are images
            frame = tk.Frame(self.top)
            frame.pack(side=tk.BOTTOM, fill=tk.X)

            self.prev_button = Button(frame, text="Previous", command=lambda: self.navigate(-1))
            self.prev_button.pack(side=tk.LEFT, padx=10, pady=10)

            self.next_button = Button(frame, text="Next", command=lambda: self.navigate(1))
            self.next_button.pack(side=tk.RIGHT, padx=10, pady=10)

            # Set initial button states
            self.update_navigation_buttons()
    def update_navigation_buttons(self):
        """Updates the state of navigation buttons based on the current image index."""
        if not self.image_paths:
        ...
        if self.current_img_index == len(self.image_paths) - 1:
            self.next_button.config(state=tk.DISABLED)
        else:
...
class Image_Thumbnail_Toolz:
    ...
    def setup_gui(self):
        ...
        self.check_recursive = Checkbutton(self.frm1, text="Recursive View", variable=f"{self.var_recursive}", command=self.update_mode_label)
        ...
        self.mode_label = tk.Label(self.frm1, text=f"{self.var_recursive}")
    ...
    def display_thumbnails(self):
        ...
        self.canvas.config(
            ...
            height=min(num_rows, 6) * thumbnail_height
        )
