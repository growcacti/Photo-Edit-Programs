...
def main(filelist=[]):
        ...
        layout=[
            ...
            [
                ...
                sg.InputText(
                    key="fontfile",
                    size=(35, 1),
                    default_text="/usr/share/fonts/truetype/freefont/FreeMonoBold.ttf",
                ),
            ],
            [
                sg.Text("output filename:"),
                sg.InputText(
                    key="outputfilename", default_text="merged_image.jpg", size=(50, 1)
                ),
            ],
            [
    ...
    layout = [
        ...
        [
            sg.Text("preview of:"),
            sg.Text(text="<no image created yet>", key="previewtext", size=(100, 1)),
        ],
        [
            sg.Image(
                filename=None,
    ...
    while True:
        ...
        if event == sg.WINDOW_CLOSED or event == "cancel":
            ...
            newfile = sg.PopupGetFile(
                message="select imagefile(s) to add",
                title="choose file",
                multiple_files=True,
            )
            if newfile is None:
            ...
            timestampstring = generate_time_stamp(
                values["suffix_date"], values["suffix_time"]
            )  # can be ""
            if values["outputfilename"] is None or len(values["outputfilename"]) == 0:
                ...
                left_part = "".join(
                    values["outputfilename"].split(".")[:-1]
                )  # the part before the last dot
...
if __name__ == "__main__":
    ...
    main(
        sys.argv[1:]
    )  # pass all other arguments to main (empty list if is passed if no arguments are given)
